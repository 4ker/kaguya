...
// tzxnote{
// will expand to namespace test_01_primitive {}
KAGUYA_TEST_GROUP_START(test_01_primitive)

using namespace kaguya_test_util;

// define func: void bool_get(kaguya::State &state);
...
{
    // just set value
    // ? value false, or false undef?
	state["value"] = true;
	TEST_EQUAL(state["value"] , true);
	TEST_CHECK(state["value"]); // true:-> nothing, false:-> print err msg
...
{
	state["value"] = 5.5f; // glad you used literal
...
// enum test is trival, it's just int
enum testenum
{
	Foo = 0,
	Bar = 1,
...
{
    // 12 numberr of "332", 42 number of "3232"
