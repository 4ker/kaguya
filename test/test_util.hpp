...
namespace kaguya_test_util
{
    ...
    inline std::vector<std::string> split(std::string s, const char delim)
    {
        ...
        std::size_t pos;
        // tzxnote: std::string::npos (-1), str.substr
    ...
    // tzxnote: TestFunctionType is a function point
    ...
    public:
        ...
        bool execute()
        {
            ...
            for (TestFunctionMapType::const_iterator it = test_functions_.begin(); it != test_functions_.end(); ++it, ++testindex)
            {
                ...
                // tzxnote: iterator is a pointer, string &ref = it->first
                const std::string& test_name = it->first;

                std::cout << test_name << "  (" << testindex << "/" << testcount << ") ...";

                // tzxnote: try-catch, run function pointer
                bool result = false;
                try
                {
                    it->second(state);
            ...
            if (fail) // failed some tests
            {
                ...
                for (size_t i = 0; i < fail_tests.size(); ++i)
                {
                    ...
                    std::cout << fail_tests[i] << ","; // fail.at(i), maybe faster
                }
...
// tzxnote: define hack, __FUNCTION__, __LINE__
// why __LINE__? because this only works with text expansion
#define TEST_CHECK(B) if(!(B)) \
    throw std::runtime_error( \
            std::string("failed.\nfunction:") +\
            __FUNCTION__  + std::string("\nline:") +\
            kaguya_test_util::to_string(__LINE__) + \
            "\nCHECKCODE:" #B ); // string concat

// tzxnote: define hack, throw runtime error(err str)
...
// tzxnote: define hack, #var -> "var", namespace hack
...
// tzxnote: test name, register to single instance
